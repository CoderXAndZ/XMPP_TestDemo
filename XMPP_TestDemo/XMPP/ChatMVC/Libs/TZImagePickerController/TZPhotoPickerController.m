...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
    UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 49, 40)];
    title.text = _model.name;
    title.textColor = XZColor(51, 51, 51);
//    title.backgroundColor = [UIColor orangeColor];
    self.navigationItem.titleView = title;
    UIButton *titleBtn = [UIButton buttonWithType:UIButtonTypeCustom];
//    titleBtn.backgroundColor = [UIColor greenColor];
    titleBtn.frame = CGRectMake(0, 0, 49, 40);
    [titleBtn setTitle:@"取消" forState:UIControlStateNormal];
    [titleBtn setTitleColor:XZColor(51, 51, 51) forState:UIControlStateNormal];
    [titleBtn addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:titleBtn];
