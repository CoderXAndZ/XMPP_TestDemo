...
@implementation XZEmoticonToolbar {
...
- (void)prepareUI {
    ...
    NSArray *packages = [XZEmoticonManager sharedManager].packages;
    
    // 创建按钮
    NSInteger index = 0;
    for (XZEmoticonPackage *package in packages) {
...
- (void)addChildButton:(NSString *)title bgImageName:(NSString *)bgImageName type:(NSInteger)type {
    ...
    [btn setBackgroundImage:[[UIImage xz_imageNamed:imageName] xz_resizableImage] forState:UIControlStateNormal];
    [btn setBackgroundImage:[[UIImage xz_imageNamed:imageNameSL] xz_resizableImage] forState:UIControlStateSelected];
