...
@implementation XZEmoticonCell {
    XZEmoticonTipView *_tipView;
    UILabel *_recentLabel;
    XZEmoticonButton *_deleteButton;
}

#pragma mark - 设置数据
- (void)setEmoticons:(NSArray<XZEmoticon *> *)emoticons {
    ...
    NSInteger index = 0;
    for (XZEmoticon *e in _emoticons) {
        XZEmoticonButton *btn = self.contentView.subviews[index++];
...
- (void)longPressed:(UILongPressGestureRecognizer *)recognizer {
    ...
    // 查找选中按钮
    XZEmoticonButton *button = nil;
    for (XZEmoticonButton *btn in self.contentView.subviews) {
...
- (void)clickEmoticonButton:(XZEmoticonButton *)button {
...
- (void)prepareUI {
    ...
    for (NSInteger i = 0; i < 21; i++) {
        ...
        UIButton *button = [XZEmoticonButton emoticonButtonWithFrame:rect tag:i];
    ...
    // 删除按钮
    _deleteButton = ((XZEmoticonButton *)self.contentView.subviews.lastObject);
    _deleteButton.deleteButton = YES;
    
    // 提示视图
    _tipView = [[XZEmoticonTipView alloc] init];
